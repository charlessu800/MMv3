...
5. Bring up a serial monitor on that serial port (baud rate doesn't matter). There are many options of software to use, but the following are what I personally use.
    ...
    * **MacOS** - [SerialTools](https://apps.apple.com/us/app/serialtools/id611021963?mt=12) or [screen](https://en.wikipedia.org/wiki/GNU_Screen) after running ls /dev/tty.* You're going to type into your terminal -> screen "whatever you identified as the correct port". If this doesn't work, it's a bit finicky. Open up serialTools to the terminal tab, change your serial port to the USB you identified, and press the connect button on the right-hand side. After, press disconnect and do the terminal instructions again.
