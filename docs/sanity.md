...
> [!NOTE]  
> If you've uploaded firmware before onto the Pi Pico, you'll need to hold the `BOOTSEL` button on the Pi Pico while plugging in. In more detail, first make sure to unplug the Pi Pico from your computer then hold the `BOOTSEL` button on the Pi Pico. Then while holding the `BOOTSEL` button plug the Pi Pico back. The Pico should appear as a  USB Mass Storage Device, RPI-RP2. You can now stop holding the `BOOTSEL` button. (For more information see: [Chapter 4. Applications information
